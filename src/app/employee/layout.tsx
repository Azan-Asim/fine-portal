...
export default function EmployeeLayout({ children }: { children: React.ReactNode }) {
    ...
    const hasEmployeeAccess = assignedRoles.some((role) => !isFullAccessRole(role));

    useEffect(() => {
        if (!isLoading) {
            if (!user) router.push('/login');
            else if (!hasEmployeeAccess) router.push('/admin/dashboard');
        }
    }, [user, isLoading, hasEmployeeAccess, router]);

    if (isLoading || !user || !hasEmployeeAccess) {
        return (
